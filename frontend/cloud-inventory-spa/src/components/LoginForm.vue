  ...
  <div class="h-screen w-screen flex flex-col justify-start items-center space-y-4">
    ...
    <div class="flex flex-col lg:w-200 h-auto bg-sky-500 border-sky-950 rounded-md p-4 shadow-sm">
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <button
          type="submit"
          class="flex sm:w-48 md:w-72 lg:w-96 items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-sky-950 text-white shadow hover:bg-sky-900/90 h-9 px-4 py-2"
        >
          Log In
        </button>
      </form>
    ...
    <div
      class="flex flex-col justify-center items-center lg:w-200 h-auto border-sky-500 border-2 rounded-md p-4 shadow-md"
    >
      <div>
        <p
          class="h-9 sm:w-48 md:w-72 lg:w-96 md:text-md lg:text-lg text-sky-950 text-center font-light"
        >
          New to Cloud Inventory?
        </p>
      </div>
      <div>
        <RouterLink
          to="/signup"
          class="flex justify-center items-center h-9 sm:w-48 md:w-72 lg:w-96 md:text-md lg:text-lg text-sky-500 text-center font-light"
      ...
      fetch('http://localhost:8080/v1/login', fetchOptions)
        ...
        .then((data) => {
          ...
          if (allTokensPresent) {
            ...
            const parsedToken = parseJwt(data.adminUiAccessToken)
